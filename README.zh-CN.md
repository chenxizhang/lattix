...
这会安装与当前平台对应的登录自启动项，并使用最新发布版本运行 `npx lattix run -d`。安装后守护进程会立即启动。在受支持的平台上无需管理员权限。

- **Windows：** 安装名为 `Lattix` 的计划任务，在登录时启动，并会在系统从睡眠或休眠唤醒后再次触发。
- **macOS：** 安装名为 `xyz.code365.lattix` 的 LaunchAgent，在登录时启动，并在安装后立即拉起守护进程。
...
这会向运行中的进程发送 SIGTERM 信号并清理 PID 文件。适用于所有运行模式（前台、守护进程、自动启动）。如果配置了自动启动，Lattix 将在下次登录时重新启动；在 Windows 的计划任务模式下，还会在从睡眠或休眠唤醒后重新触发。
