...
The project README SHALL include a dedicated section communicating Lattix's security and compliance advantages.

#### Scenario: README contains security section
- **WHEN** a user reads the project README
- **THEN** they SHALL find a section that explains Lattix requires no tunnels, no exposed ports, and does not move any data
...
- **THEN** they SHALL see text explaining that Lattix requires no tunnels, no exposed ports, and moves no data
...
- **THEN** the login card SHALL display the text "Distributed agent orchestration" below the Lattix heading
