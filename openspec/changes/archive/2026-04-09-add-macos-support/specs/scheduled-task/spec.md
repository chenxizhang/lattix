...
- **WHEN** the user runs `lattix install` on Windows and the `Lattix` scheduled task already exists
...
- **WHEN** the user runs `lattix install` on Windows and task creation fails
...
- **WHEN** the computer resumes from sleep or hibernation on Windows and the `Lattix` scheduled task is installed
...
On Windows, the system SHALL provide a `lattix uninstall` command implementation that stops the running Lattix instance (if any) and removes the scheduled task.
...
- **WHEN** the user runs `lattix uninstall` on Windows and no `Lattix` scheduled task exists
...
On Windows, the system SHALL provide the ability to query whether a `Lattix` scheduled task is registered.

#### Scenario: Query installed task
- **WHEN** the system queries scheduled-task state on Windows and the `Lattix` task exists
- **THEN** the query SHALL return an installed state

#### Scenario: Query non-existent task
- **WHEN** the system queries scheduled-task state on Windows and no `Lattix` task exists
