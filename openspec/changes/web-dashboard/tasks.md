...
- [ ] 3.1 Write tests for auth module: MSAL config initialization, login redirect trigger, token acquisition, logout, unauthenticated state detection, token expiry → silent refresh → login redirect fallback
...
- [ ] 4.1 Write tests for graph module: list tasks, read task content, list results for a task, read result content, discover nodes from results, submit task, pagination, error handling for 401/403/404/429/5xx, conflict behavior on PUT (all with mocked fetch)
...
- [ ] 5.1 Write tests for router: hash change dispatches correct handler, route parameters extracted, unknown routes fall back to home
...
- [ ] 6.1 Write tests for home view: renders node list when data exists, renders onboarding when empty, renders submit form
...
- [ ] 6.3 Write tests for task list view: renders task items, shows status per machine, handles empty state
...
- [ ] 6.5 Write tests for task detail view: renders task metadata and per-machine results
...
- [ ] 10.1 Write tests for input sanitization: shell metacharacters escaped, max lengths enforced, restricted fields stripped
...
- [ ] 10.3 Write test verifying CSP meta tag is present in built `index.html`
- [ ] 10.4 Write test verifying no inline `<script>` tags exist in `index.html`
- [ ] 10.5 Write tests for account validation flow: workspace exists → proceed, workspace missing → onboarding with account hint, switch account → re-login with prompt
- [ ] 10.6 Write tests for Graph API error handling: 401 → refresh + retry, 403 → consent message, 429 → retry-after, 5xx → error + retry button
- [ ] 10.7 Write tests for task ID collision prevention: conflictBehavior header sent, 409 → regenerate ID + retry
...
- [ ] 11.2 Commit and push to `main` — verify GitHub Actions workflow runs and deploys to `https://lattix.code365.xyz/`
- [x] 11.3 Add `.gitignore` entries: `web/node_modules/`, `web/dist/`
- [ ] 11.4 Update `README.md`: add Web Dashboard section documenting — what it does, how to access it at `https://lattix.code365.xyz/`, security model, mobile install instructions
- [ ] 11.5 Document Entra ID app registration in README (for reference — the app is already created by task 1.1)
- [ ] 11.6 Final build verification: `cd web && npm run build` succeeds with no errors
- [ ] 11.7 Final test verification: `cd web && npm test` — all unit tests pass
