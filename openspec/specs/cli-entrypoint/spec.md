...
- **THEN** the system SHALL create a task JSON file in `~/.agentbroker/tasks/` with a generated unique ID, the provided prompt, the working directory, and the current machine hostname as `createdBy`
...
- **WHEN** the user runs `agentbroker submit --prompt "..." --agent "claude -p {prompt} --allowedTools WebSearch"`
...
- **THEN** the system SHALL read all task files from `~/.agentbroker/tasks/`, display each task's ID, title, and execution results from each machine, formatted as a table or list
